...
class ForwardKinematics:
    ...
    def get7DOFBaseRef(self, *thetas):
        ...
        #Kinematic Dimensions and Home Configuration
        a1, a2, a3, a4, a5 = 125.83, 115.83, 97.52, 71.64, 172.52
        homeConfiguration = [85.51, 180.18, 177.14, 183.23, 183.23, 180.18, 174.07, 146.61]
        ...
        theta2 = (thetas[1]-homeConfiguration[1])*(pi/180.0)
        theta3 = (thetas[2]-homeConfiguration[2])*(pi/180.0)
        theta4 = (thetas[3]-homeConfiguration[3])*(pi/180.0) #The homeConfiguration variable offsets theta by a given amount in order to change where the servo's 0 degree point starts. Note: homeConfigurationOffset may not be necessary for every application
        theta5 = (thetas[4]-homeConfiguration[4])*(pi/180.0)
        theta6 = (thetas[5]-homeConfiguration[5])*(pi/180.0)
        theta7 = (thetas[6]-homeConfiguration[6])*(pi/180.0) #The homeConfiguration variable offsets theta by a given amount in order to change where the servo's 0 degree point starts. Note: homeConfigurationOffset may not be necessary for every application
        theta8 = (thetas[7]-homeConfiguration[7])*(pi/180.0)
        
        #Forward Kinematics
        X = round(cos(theta1)*(cos(theta2)*(sin(theta4)*(cos(theta5)*(-a5*cos(theta6)-a4)-a3)-a5*cos(theta4)*sin(theta6))+sin(theta2)*(cos(theta3)*(cos(theta4)*(cos(theta5)*(-a5*cos(theta6)-a4)-a3)+a5*sin(theta4)*sin(theta6)-a2)+sin(theta3)*sin(theta5)*(a5*cos(theta6)+a4)-a1))+sin(theta1)*(cos(theta3)*sin(theta5)*(-a5*cos(theta6)-a4)+sin(theta3)*(cos(theta4)*(cos(theta5)*(-a5*cos(theta6)-a4)-a3)+a5*sin(theta4)*sin(theta6)-a2)), 2)
        Y = round(cos(theta1)*(cos(theta3)*sin(theta5)*(a5*cos(theta6)+a4)+sin(theta3)*(cos(theta4)*(cos(theta5)*(a5*cos(theta6)+a4)+a3)-a5*sin(theta4)*sin(theta6)+a2))+sin(theta1)*(cos(theta2)*(sin(theta4)*(cos(theta5)*(-a5*cos(theta6)-a4)-a3)-a5*cos(theta4)*sin(theta6))+sin(theta2)*(cos(theta3)*(cos(theta4)*(cos(theta5)*(-a5*cos(theta6)-a4)-a3)+a5*sin(theta4)*sin(theta6)-a2)+sin(theta3)*sin(theta5)*(a5*cos(theta6)+a4)-a1)), 2)
        Z = round(cos(theta2)*(cos(theta3)*(cos(theta4)*(cos(theta5)*(a5*cos(theta6)+a4)+a3)-a5*sin(theta4)*sin(theta6)+a2)+sin(theta3)*sin(theta5)*(-a5*cos(theta6)-a4)+a1)+sin(theta2)*(sin(theta4)*(cos(theta5)*(-a5*cos(theta6)-a4)-a3)-a5*cos(theta4)*sin(theta6)), 2)
        
        #X, Y, Z positions and max reach for X, Y, Z
        positions = [X, Y, Z]  
        maximumReachXYZ = [a1+a2+a3+a4+a5, a1+a2+a3+a4+a5, a1+a2+a3+a4+a5]
        ...
        return [positions[0], positions[1], positions[2], maximumReachXYZ, [a1, a2, a3, a4, a5], homeConfiguration]
