...
import inspect
import kinematics
import IP_movement_algorithms
import main.utility_package.utilities as utilities
import callback_functions as CF
import main.gui_components.callback_functions as genericCF
...
GUIAndRAMStrings = ("positionCheckBox", "speedCheckBox", "voltageCheckBox", "loadCheckBox", "temperatureCheckBox",
...
savedServoModel = previous_state_logging_system.Log('gui_components/roboarm/_Saved_Values_/Last_Servo_Model_Used.txt').getParameters("lastServoModel") #Last used servo model
savedValues = previous_state_logging_system.Log('gui_components/roboarm/_Saved_Values_/Saved_{}_GUI&RAM_Values.txt'.format(savedServoModel.lastServoModel)).getParameters(*GUIAndRAMStrings) #Gets config params based on last used servo model
...
def _updateRAMSlidersFromServoModelFile(): #Only used in the updateGUIConfigSliders function
    ...
    savedValues = previous_state_logging_system.Log('gui_components/roboarm/_Saved_Values_/Saved_{}_GUI&RAM_Values.txt'.format(window.dropBoxServoModelVariable.get())).getParameters(*GUIAndRAMStrings)
...
def _displayHideAllConfigSliders(): #Only used in the updateGUIConfigSliders function
    ...
    if window.dropBoxVariable.get() != previosDropBoxVariable:
        if window.dropBoxVariable.get() == "Configuration 1":
            window.statusInterfaceFrame.grid_remove()
        ...
        elif window.dropBoxVariable.get() == "Status Interface":
            window.statusInterfaceFrame.grid()
            window.config3Frame.grid_remove()
            window.config2Frame.grid_remove()
            window.config1Frame.grid_remove()
...
def roboarmControllerUpdate(imageProcessingData):
...
def roboarmControllerTab(window, notebook):
    roboArmTab = Tkinter.Frame(notebook, width=1000, height=200)
    roboArmTab.place()
    notebook.add(roboArmTab, text="RoboArm")
    
    #EVENT HANDLER WHEN USER PRESSES ESCAPE
    window.bind("<Key>", CF.escape)
    
    #TRACK OBJECT BUTTON
    trackObjectButton = Tkinter.Button(window, text="Track Object", command = CF.trackObject, width = 11)
    trackObjectButton.place(relx = 0.5, rely = 0.435, anchor = "center")
    ...
    scalePositionFrame = Tkinter.Frame(roboArmTab)
    ...
    dropBoxServoNum.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    dropBoxServoModel.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    dropBox = Tkinter.OptionMenu(scalePositionFrame, dropBoxVariable, "Configuration 1", "Configuration 2", "Configuration 3", "Status Interface")
    dropBox.grid(row = 2, column = 0, columnspan = 4, padx = 2, pady = 3)
    dropBox.config(width = 36)
    dropBox.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    importRamButton.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    exportRamButton.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    dropBoxFwdKinematicSelector.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    dropBoxInvKinematicSelector.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    dropBoxInvKinematicSolutionMenu.bind("<Button-1>", lambda event, args=[window]: genericCF.Refresh(event, args))
    ...
    individualServoControlFrame = Tkinter.Frame(roboArmTab)
    ...
    #STATUS INTERFACE FRAME
    statusInterfaceFrame = Tkinter.Frame(roboArmTab)
    statusInterfaceFrame.grid(row = 0, column = 2, padx = 40, sticky = "N")
    ...
    printCheckBoxes = []
    for x in range(5):
        printCheckBoxes.append(Tkinter.Checkbutton(statusInterfaceFrame, text=names[x], variable = printCheckBoxValArray[x]))
    ...
    #Sample Rate
    sampleRateLabel = Tkinter.Label(statusInterfaceFrame, text = "Sample Rate (Hz):")
    sampleRateLabel.grid(row = 0, column = 2, sticky = "E")
    samplingRateText = Tkinter.StringVar()
    samplingRateEntry = Tkinter.Entry(statusInterfaceFrame, textvariable = samplingRateText, width = 8, justify = "center"); samplingRateText.set(savedValues.sampleRate)
    samplingRateEntry.bind("<Return>", CF.samplingRateOk)
    samplingRateEntry.grid(row = 0, column = 3, sticky = "W")
    sampleRateOkButton = Tkinter.Button(statusInterfaceFrame, text = "OK", command = CF.samplingRateOk)
    sampleRateOkButton.grid(row = 0, column = 3, sticky = "E")
    setattr(window, "samplingRateText", samplingRateText)
    
    #Console
    consolText = Tkinter.Text(statusInterfaceFrame, borderwidth = 7, width = 100, height = 18)
    ...
    #record/play back
    recordButton = Tkinter.Button(statusInterfaceFrame, text="Record", command = lambda: CF.RecordJointAngles(), width = 12)
    recordButton.grid(row = 6, column = 1)
    recordButton.config(state="disabled")
    
    playButton = Tkinter.Button(statusInterfaceFrame, text="Play", command = CF.playRecordedJointAnglesFile, width = 12)
    playButton.grid(row = 6, column = 2)
    playButton.config(state="disabled")
    
    playbackFileNameText = Tkinter.StringVar()
    playBackFileName = Tkinter.Label(statusInterfaceFrame, text = "File Name:") 
    playBackFileName.grid(row = 6, column = 3, sticky = "E")
    fileNameTextEntry = Tkinter.Entry(statusInterfaceFrame, textvariable=playbackFileNameText, state="readonly")
    fileNameTextEntry.grid(row = 6, column = 4, sticky = "EW")
    
    loadButton = Tkinter.Button(statusInterfaceFrame, text="Load", command = CF.loadRecordedJointAnglesFile)
    loadButton.grid(row = 6, column = 5, sticky = "W", padx = 10)
    
    playBackRateLabel = Tkinter.Label(statusInterfaceFrame, text = "Playback Rate:", font = "Helvetica 10 bold")
    minusPlayBackButton = Tkinter.Button(statusInterfaceFrame, width = 2, text = "-", font = "Helvetica 15 bold", command = CF.playBackRateMinus)
    plusPlayBackButton = Tkinter.Button(statusInterfaceFrame, width = 2, text = "+", font = "Helvetica 15 bold", command = CF.playBackRatePlus)
    ...
    setattr(window, "statusInterfaceFrame", statusInterfaceFrame)
    
    #CONFIG 3 FRAME
    config3Frame = Tkinter.Frame(roboArmTab)
    ...
    #CONFIG 2 FRAME
    config2Frame = Tkinter.Frame(roboArmTab)
    ...
    #CONFIG 1 FRAME
    config1Frame = Tkinter.Frame(roboArmTab)
    ...
    #SET CALLBACK FUNCTIONS WINDOW OBJECT
